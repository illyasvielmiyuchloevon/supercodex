...
5. 每次提问前必须读取并更新 `.supercodex/FINAL_GOAL.md` 和 `.supercodex/AUTO_DEV_STATE.json`，确保不会超过 10 个问题。
6. 一旦进入 PRD、架构、Plan 或执行阶段，**绝对禁止再向用户提问、请求确认或等待用户决策**；必须自行查文件、读日志、搜索代码、运行命令、修复问题并继续推进。
7. 如果已经问满 10 个问题，仍有未澄清事项，必须转化为合理假设后进入 PRD 阶段。
...
3. 覆盖关系必须由 `.supercodex/FINAL_GOAL.md`、`.supercodex/PRD.md`、`.supercodex/ARCHITECTURE.md` 和 `.supercodex/PLAN.md` 表达；`.supercodex/AUTO_DEV_STATE.json` 只记录 SuperCodex 可读取的状态参数。
4. 每次完成 Plan 所有任务后，必须执行最终目标验收。验收失败时，不得结束；必须更新 PRD、架构和 Plan，并进入下一轮。
5. 只有当 FINAL_GOAL、PRD、ARCHITECTURE、PLAN 已完成覆盖核对，且 `.supercodex/AUTO_DEV_STATE.json` 的质量、验收和交付状态参数均显示通过时，才允许最终交付。

### 1.4 外部循环器兼容规则

1. 外部循环器断点续跑状态必须使用 `.supercodex/AUTO_DEV_STATE.json`。
2. AUTO_DEV_STATE 只保存机器可读状态参数；目标、PRD、架构和 Plan 使用 Markdown。
3. 每次启动先读取 AUTO_DEV_STATE、FINAL_GOAL、PRD、ARCHITECTURE、PLAN 和必要 runtime 状态，再从断点继续。
4. AUTO_DEV_STATE 不存在时创建并初始化为 Phase 0。
5. 阶段切换、任务完成、测试失败、修复完成、验收失败、验收通过时更新 AUTO_DEV_STATE。
6. AUTO_DEV_STATE 必须保持合法 JSON，优先使用原子写入。
...
2. Stage、Phase、Milestone、commit 或 push 变化不单独触发 fresh thread。
3. PLAN 当前 Cycle 全部完成后，才进入全项目 Phase 6 验收。
4. Phase 6 失败时更新 PRD、ARCHITECTURE、PLAN 和 AUTO_DEV_STATE，创建下一 Cycle。
...
4. 主 Agent 不得把最终责任交给子代理；必须整合、复核、测试并按需更新 `.supercodex/FINAL_GOAL.md`、`.supercodex/PRD.md`、`.supercodex/ARCHITECTURE.md`、`.supercodex/PLAN.md` 和 `.supercodex/AUTO_DEV_STATE.json`。
5. 子代理结果必须转化为可验证交付物：代码修改、测试结果、审查发现、根因结论或明确的不可行原因。
6. 如果子代理发现 PRD、架构、Plan、测试或最终验收缺口，主 Agent 必须按闭环规则更新对应文件或状态参数，而不是只记录意见。
...
1. 仅在 Phase 0 且确实阻塞推进时提问。
2. 每个问题和回答必须写入 `.supercodex/FINAL_GOAL.md` 的“澄清记录与回答”；`.supercodex/AUTO_DEV_STATE.json` 只更新澄清状态和计数。
...
5. 如果没有阻塞问题，或已达到 10 个问题上限，必须关闭澄清阶段，设置 `clarification.status = "CLOSED"`，进入 Phase 1。
...
4. 普通 Stage 完成后只更新 PLAN/AUTO_DEV_STATE 并继续；稳定边界可以形成阶段性 commit/push。
5. PLAN 不包含最终验收或最终交付任务；它们在 PLAN 完成后由标准流程单独执行。
...
最终目标覆盖关系由 FINAL_GOAL、PRD、ARCHITECTURE 和 PLAN 的必要说明表达；AUTO_DEV_STATE 只保存 SuperCodex 可读取的真实状态参数。
...
3. 代码必须模块化、可测试、可维护。
4. 新增或修改代码必须符合项目现有风格。
5. 必须同时更新与实现直接相关的配置、类型定义、迁移脚本、测试夹具等配套内容。
...
代码审查完成后，把机器状态写入 `.supercodex/AUTO_DEV_STATE.json` 的 `quality.code_review_status`，并按项目风险重点检查：
...
2. Phase 6 必须重新核对最终目标、PRD、架构、PLAN、AUTO_DEV_STATE、测试、代码审查和交付准备，而不是信任 Plan checklist 已完成。
3. Phase 6 PASS 后才能进入 Phase 7；Phase 6 FAIL 时必须回到 Phase 1，更新 PRD、ARCHITECTURE、PLAN 和 AUTO_DEV_STATE，创建下一 Cycle。
...
最终验收结果必须写入 `.supercodex/AUTO_DEV_STATE.json`：
...
6. 更新 `.supercodex/AUTO_DEV_STATE.json` 的质量、验收和下一步状态参数。
7. 继续执行新 Plan。
...
3. 按 AUTO_DEV_STATE 的 phase、plan.current_task_id 和 execution.next_action 继续。
4. PLAN 当前 Cycle 完成后进入 Phase 6；acceptance=PASS 后进入 Phase 7。
5. 每次退出前更新 AUTO_DEV_STATE 的下一步状态。
```
