...
5. 一旦进入 PRD、架构、Plan 或执行阶段，**绝对禁止再向用户提问、请求确认或等待用户决策**；必须自行查文件、读日志、搜索代码、运行命令、修复问题并继续推进。
6. 如果已经问满 10 个问题，仍有未澄清事项，必须转化为合理假设后进入 PRD 阶段。
...
5. AUTO_DEV_STATE 表达当前 Phase、下一步 Phase 动作与最终验收/交付状态。
...
4. PLAN 内部进度写入 PLAN 或相关交付物；AUTO_DEV_STATE 描述当前 Phase 调度。
5. PLAN 不包含最终验收或最终交付任务；它们在 PLAN 完成后由标准流程单独执行。
6. PLAN 应整体具备执行方向、验证闭环和追踪关系；不要把这些要求下压成每个任务的硬性字段或硬性属性。
7. PLAN 内 checklist 表达执行组织与完成状态；AUTO_DEV_STATE 表达 Phase 级调度状态。
...
2. 在同一 Phase 内连续执行 PLAN 中可推进的工作，由 Codex 自行依据 PLAN 组织执行节奏。
3. 执行进度记录到 `.supercodex/PLAN.md` 的 checklist 或相关交付物；`.supercodex/AUTO_DEV_STATE.json` 保持 Phase 级调度状态。
4. 代码必须模块化、可测试、可维护。
5. 新增或修改代码必须符合项目现有风格。
...
代码审查完成后，按项目风险重点检查；审查结果作为 Phase 5 证据保存在测试输出、PLAN 记录或相关交付物中，并在 Phase 收敛时同步 AUTO_DEV_STATE 的 Phase 调度结论。
...
最终验收结果属于 Phase 6 -> Phase 7 或 Phase 6 -> 下一 Cycle 的调度边界，必须写入 `.supercodex/AUTO_DEV_STATE.json`：
