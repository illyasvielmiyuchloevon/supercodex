...
export function SessionTranscript(props: {
  ...
  return (
    ...
    >
      <box height={1} />
      <ShowStructuredMessages messages={props.messages ?? []} lines={props.lines} theme={props.theme} />
    </scrollbox>
  );
}

function ShowStructuredMessages(props: { messages: TuiMessage[]; lines: string[]; theme: AgentTuiTheme }) {
  return (
    <Show
      ...
      fallback={
        <For each={props.lines.length > 0 ? props.lines : ["Waiting for Codex app-server events..."]}>
          {(line, index) => <TranscriptLine line={line} index={index()} theme={props.theme} />}
        </For>
      }
    >
      <MessageList messages={props.messages} theme={props.theme} />
    </Show>
  );
}

function TranscriptLine(props: { line: string; index: number; theme: AgentTuiTheme }) {
  ...
  return (
    <box
      id={`transcript-line-${props.index}`}
