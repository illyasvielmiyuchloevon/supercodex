...
普通文本是普通任务。它可以使用 SuperCodex 的需求分析、计划、执行、测试和审查流程，但不会被保存为项目最终目标，也不会进入最终目标的 Phase 6/Phase 7 循环。只有输入 `/goal <prompt>` 时，才会重置 `.supercodex` 状态并启动最终目标交付循环。
...
- `/new <prompt>` 创建新的普通任务 run 并立刻发送 prompt。
- `/clear` 是 `/new` 的别名。

`/goal <prompt>` 是最终目标入口：

- 重置当前项目过时的 `.supercodex` 状态。
- 将 prompt 写入 `.supercodex/FINAL_GOAL.md`。
- 启用完整 PRD / 架构 / PLAN / Phase 6 / Phase 7 交付循环。
...
/new [prompt]         新建普通任务 session
/goal <prompt>        重置状态并启动最终目标交付循环
/model <name>         设置下一轮模型
...
.supercodex/
  AUTO_DEV_STATE.json
  TASK.md                  # 普通任务模式
  FINAL_GOAL.md            # 显式 /goal 模式
  CLARIFICATIONS.md        # 显式 /goal 模式
  ASSUMPTIONS.md           # 显式 /goal 模式
  PRD.md                   # 显式 /goal 模式
  ARCHITECTURE.md          # 显式 /goal 模式
  PLAN.md
  TRACEABILITY_MATRIX.md   # 显式 /goal 模式
  CODE_REVIEW_REPORT.md
  FINAL_ACCEPTANCE_REPORT.md # 显式 /goal 模式
  progress.md
  checkpoints.md
  last-action.md
  runtime/
